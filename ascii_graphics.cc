...
card_template_t operator+(const card_template_t& first, const card_template_t& next){
  if (first.size() == 0){
    return next;
  }
  if (next.size() == 0){
  ...
  card_template_t res{};
    for (size_t i = 0; i < first.size(); i++){
        res.push_back(first[i]+next[i]);
    }
    return res;
}

card_template_t append(const card_template_t & c1, const card_template_t & c2){
...
card_template_t add_border(const card_template_t& c){
  card_template_t result{};
  std::string border_left_right {};
  for (size_t i = 0; i < c.at(0).size()/EXTERNAL_BORDER_CHAR_LEFT_RIGHT.size(); i++){
    border_left_right = border_left_right + EXTERNAL_BORDER_CHAR_LEFT_RIGHT;
  }
  card_template_t border_up_down {};
  for (size_t i = 0; i < c.size(); i++){
    border_up_down.push_back(EXTERNAL_BORDER_CHAR_UP_DOWN);
  }
  card_template_t upper{};
  upper.push_back(EXTERNAL_BORDER_CHAR_TOP_LEFT + border_left_right + EXTERNAL_BORDER_CHAR_TOP_RIGHT);
  card_template_t lower{};
  lower.push_back(EXTERNAL_BORDER_CHAR_BOTTOM_LEFT + border_left_right + EXTERNAL_BORDER_CHAR_BOTTOM_RIGHT);

  result = border_up_down + c + border_up_down;
  result = append(append(upper,result),lower);
  return result;
}

card_template_t add_left_right_border(const card_template_t & c){
  card_template_t result{};
  card_template_t border_up_down {};
  for (size_t i = 0; i < c.size(); i++){
    border_up_down.push_back(EXTERNAL_BORDER_CHAR_UP_DOWN);
  }
  return border_up_down + c + border_up_down;
}

card_template_t add_top_bottom_border(const card_template_t& c){
  std::string border_left_right {};
  size_t max_len = 0;
  for (auto i : c){
    if (max_len < i.size()){
  ...
  for (size_t i = 0; i < max_len/EXTERNAL_BORDER_CHAR_LEFT_RIGHT.size() - 2; i++){
    border_left_right = border_left_right + EXTERNAL_BORDER_CHAR_LEFT_RIGHT;
  }
  card_template_t upper{};
  upper.push_back(EXTERNAL_BORDER_CHAR_TOP_LEFT + border_left_right + EXTERNAL_BORDER_CHAR_TOP_RIGHT);
  card_template_t lower{};
  lower.push_back(EXTERNAL_BORDER_CHAR_BOTTOM_LEFT + border_left_right + EXTERNAL_BORDER_CHAR_BOTTOM_RIGHT);
  return append(append(upper,c),lower);
}

std::ostream& operator<<(std::ostream& os, const card_template_t& c){
  for (auto i : c){
    os << i << std::endl;
  }
  return os;
...
card_template_t display_player_card(int player_num,const std::string & name,int life,int mana) {
  std::ostringstream oss;
